...
plot.periodicCycle = function(data, register, standardized){
  ...
  while (i+2<=length(z_idx)){
    if (standardized==0){
      if(register==0){
        tmp=data.frame(cycle=i, time=seq(z_idx[i],z_idx[i+2]), y_value=smoothed_curve[z_idx[i]:z_idx[i+2]])
      }
      else{
        tmp=data.frame(cycle=i, time=seq(1,length(seq(z_idx[i],z_idx[i+2]))), y_value=smoothed_curve[z_idx[i]:z_idx[i+2]])  
      }
      result=rbind(result,tmp)
    }
    else{
      if(register==0){
        tmp=data.frame(cycle=i, time=scale(seq(z_idx[i],z_idx[i+2])), y_value=smoothed_curve[z_idx[i]:z_idx[i+2]])
      }
      else{
        tmp=data.frame(cycle=i, time=scale(seq(1,length(seq(z_idx[i],z_idx[i+2])))), y_value=smoothed_curve[z_idx[i]:z_idx[i+2]]) 
      }
      result=rbind(result,tmp)
    }
    i=i+2
