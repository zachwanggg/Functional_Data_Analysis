---
title: "HC & PD nodes mean curves comparison"
author: "Zach Wang"
date: "9/19/2020"
output:
...
---
# Version nodes: use original TS and analyze the **mean curve** of each node across different patients corresponding to the 4 control groups
...
mean.df <- function(patient_df){
  ...
  for(node in 1:ncol(patient_df)){
    rotpcalist = fPCA.nodes(data_mat=patient_df, k=11, nharm=2, plt=0)
    # PC1 & PC2
    harmfd <- rotpcalist[[1]]
...
nodes.compare <- function(patient_list, node_number){
  ...
  names(nodes_df)=paste('patient', patient_list, sep = "")
...
## 2.1. Compare same node across different patients from PD-placebo group
```{r}
pd_placebo <- c(1,4,6,10,14,15,18,20,22,23,31,46,47,50,56,60,61,64,65,80,81,91,97)
patient1 = ReadFile(paste('/Users/hanwang/desktop/Git_desktop/Functional_Data_Analysis/Data for Zach/Data for Zach ', 1,'.csv', sep=""))
PC1_df = data.frame(matrix(nrow=600))
PC2_df = data.frame(matrix(nrow=600))
mean_df = data.frame(matrix(nrow=600))
for (i in c(1:32)){
  # one for each node from PD-placebo group together
  node.df = nodes.compare(pd_placebo, node_number=i)
  rotpcalist = fPCA.nodes(data_mat=as.matrix(node.df), k=11, nharm=2, plt=0)
  # PC1 & PC2 & Mean
  harmfd <- rotpcalist[[1]]
  basisfd <- harmfd$basis
  rangex <- basisfd$rangeval
...
#plot
tmp = as.matrix(mean_df)
d <- melt(data = tmp, id.vars = c("index"), measure.vars = colnames(mean_df))
print(ggplot(d, aes(x = Var1, y = value)) + 
          geom_line(aes(color = Var2)) + ggtitle("PD-placebo group node mean curves"))
```

## 2.2. Compare same node across different patients from HC-placebo group
```{r}
...
for (i in c(1:32)){
  # one for each node from PD-placebo group together
  node.df = nodes.compare(hc_placebo, node_number=i)
  rotpcalist = fPCA.nodes(data_mat=as.matrix(node.df), k=11, nharm=2, plt=0)
  # PC1 & PC2 & Mean
  harmfd <- rotpcalist[[1]]
  basisfd <- harmfd$basis
...
d <- melt(data = tmp, id.vars = c("index"), measure.vars = colnames(mean_df))
print(ggplot(d, aes(x = Var1, y = value)) + 
          geom_line(aes(color = Var2)) + ggtitle("HC-placebo group node mean curves"))
```

## 2.3. Compare same node across different patients from PD-Ldopa group
```{r}
pd_ldopa <- c(2,3,5,8,9,13,16,17,19,21,24,32,35,45,48,49,54,55,59,62,63,66,76,78,79,82,84,87,92,98)
patient1 = ReadFile(paste('/Users/hanwang/desktop/Git_desktop/Functional_Data_Analysis/Data for Zach/Data for Zach ', 1,'.csv', sep=""))
PC1_df = data.frame(matrix(nrow=600))
PC2_df = data.frame(matrix(nrow=600))
mean_df = data.frame(matrix(nrow=600))
for (i in c(1:32)){
  # one for each node from PD-placebo group together
  node.df = nodes.compare(pd_ldopa, node_number=i)
  rotpcalist = fPCA.nodes(data_mat=as.matrix(node.df), k=11, nharm=2, plt=0)
  # PC1 & PC2 & Mean
  harmfd <- rotpcalist[[1]]
  basisfd <- harmfd$basis
...
d <- melt(data = tmp, id.vars = c("index"), measure.vars = colnames(mean_df))
print(ggplot(d, aes(x = Var1, y = value)) + 
          geom_line(aes(color = Var2)) + ggtitle("PD-ldopa group node mean curves"))
```

## 2.4. Compare same node across different patients from HC-Ldopa group
```{r}
hc_ldopa <- c(12,26,27,30,34,37,39,42,44,52,57,68,69,72,74)
patient1 = ReadFile(paste('/Users/hanwang/desktop/Git_desktop/Functional_Data_Analysis/Data for Zach/Data for Zach ', 1,'.csv', sep=""))
PC1_df = data.frame(matrix(nrow=600))
PC2_df = data.frame(matrix(nrow=600))
mean_df = data.frame(matrix(nrow=600))
for (i in c(1:32)){
  # one for each node from PD-placebo group together
  node.df = nodes.compare(c(12,26,27,30,34,37,42,44,52,57,68,69,72,74), node_number=i)
  rotpcalist = fPCA.nodes(data_mat=as.matrix(node.df), k=11, nharm=2, plt=0)
  # PC1 & PC2 & Mean
  harmfd <- rotpcalist[[1]]
  basisfd <- harmfd$basis
...
d <- melt(data = tmp, id.vars = c("index"), measure.vars = colnames(mean_df))
print(ggplot(d, aes(x = Var1, y = value)) + 
          geom_line(aes(color = Var2)) + ggtitle("HC-ldopa group node mean curves"))
```
